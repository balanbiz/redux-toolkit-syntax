...
import { toggleTodoCheckboxAsync, removeTodoAsync } from "../state/todoSlice";
...
const TodoItem: React.FC<ITodo> = memo(({ id, text, completed }) => {
    ...
    return (
        <li>
            ...
            <input type="checkbox" checked={completed} onChange={() => dispatch(toggleTodoCheckboxAsync(id))} />
            <span>{text}</span>
            <span onClick={() => dispatch(removeTodoAsync(id))} className="delete">
                &times;
            </span>
        </li>
    );
